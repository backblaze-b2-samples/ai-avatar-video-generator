...
## Purpose
Upload files from the browser to Backblaze B2 with real-time progress tracking. This is the
generic upload surface kept from the starter kit (lands files under `uploads/`); it is
separate from the avatar-video render flow, which writes takes under `avatar-projects/`.
