...
## Purpose
List, preview, download, and delete files stored in Backblaze B2. This is the kept
full-bucket explorer from the starter kit — it browses the entire bucket (including
`avatar-projects/` and `uploads/`). For a project-scoped view with inline video playback,
see the [Scoped Library](library.md).
